...
import style from "./DesktopsAll.module.scss"

const DesktopsAll: React.FC = () => {
    const dispatch = useAppDispatch()
    const {goods, status} = useSelector((state:RootState) => state.goodsSlice)
    const desktopsGoods = goods.filter((item: goodsType) => item.type === "desktops")
    const skeleton = [...new Array(5)].map((_, index) => <Skeleton key={index}/>)
    const categoryList:string[] = ["CUSTOM Desktops", "MSI ALL-IN-ONE Desktops", "HP/COMPAQ Desktops"]
    ...
    let filteredGoods = desktopsGoods;
    ...
    const filterGoodsByPriceRange = (minPrice: number, maxPrice: number) => {
        return desktopsGoods.filter((good) => {
    ...
    return (  
        <section className={style.root}>
             <div className={style.path}>
                ...
                <Link to={"/techland-store/desktops"}>Desktops</Link>
            </div>
            <h3 className={style.title}>Desktops</h3>
                        ...
                        <div className={style.category}>
                            ...
                            <div className={style.category__content}>
                                <ul className={style.category__list}>
                                    ...
                                    ((category === "CUSTOM Desktops" || category === "MSI ALL-IN-ONE Desktops") || category === "HP/COMPAQ Desktops") && (
                                ...
                                <ul className={style.category__list_numbers}>
                                    <li>{desktopsGoods.filter(good => good.category === "CUSTOM Desktops").length}</li>
                                    <li>{desktopsGoods.filter(good => good.category === "MSI ALL-IN-ONE Desktops").length}</li>
                                    <li>{desktopsGoods.filter(good => good.category === "HP/COMPAQ Desktops").length}</li>
                                </ul>
                            </div>
                        </div>
